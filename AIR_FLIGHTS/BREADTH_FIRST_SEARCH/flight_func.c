...
void isflight( fl_db_ptr casted_ptr, char* from, char* to )
{
	...
	while(dist=find(casted_ptr, from, anywhere))
	{
		if( d=match( casted_ptr, anywhere, to ) )
		{
			push( casted_ptr, from, to, dist );
			push( casted_ptr, anywhere, to, d );
			return;
		}
	}
...
void print_result_func( print_result_iface_ptr ptr )
{
	...
	{
		printf("%s to ", (*casted_ptr).fl_stack[i].from);
		distance += (*casted_ptr).fl_stack[i].distance;
	}
	printf("%s\n", (*casted_ptr).search_to);
	printf("From %s to %s summ distance = %d.\n", 
